    ...
    return <div className='flex flex-col gap-3 w-[100%]'>
        ...
        {experiences.map((experience, key) => {
            return <div key={key} className='flex items-center space-between gap-1 w-[100%] text-right'>
                <p className='font-bold text-sm w-[50%] text-center'>{experience.date}</p>
                <p className='font-bold text-sm w-[100%] text-center'>{experience.role}</p>
                <p className='text-sm text-center max-w-[90px]'>{experience.company}</p>
      ...
      {object.education.map((education, key) => {
        return (
          ...
          >
            ...
            <p className="text-sm text-center max-w-[100%]">{education.date}</p>
          </div>
        );
      })}
    </div>
